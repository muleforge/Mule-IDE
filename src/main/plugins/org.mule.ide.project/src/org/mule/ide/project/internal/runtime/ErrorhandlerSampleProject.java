...
public class ErrorhandlerSampleProject extends MuleSampleProject {
	...
	private static final String ACTIVEMQ_URL = "http://people.apache.org/repo/m2-incubating-repository/org/apache/activemq/activemq-core/4.1.0-incubator/" + ACTIVEMQ_JAR;
	...
	private static final String CONCURRENT_URL = "http://repo1.maven.org/maven2/concurrent/concurrent/1.3.4/" + CONCURRENT_JAR;
	...
	private void downloadUrlToFile(String url, final File destinationFile) throws CoreException {
		...
		IRetrieveFileTransferContainerAdapter fileTransferContainer = 
			(IRetrieveFileTransferContainerAdapter)container.getAdapter(IRetrieveFileTransferContainerAdapter.class);
		if (fileTransferContainer != null) {
			...
			IFileID remoteFileID = FileIDFactory.getDefault().createFileID(fileTransferContainer.getRetrieveNamespace(), url);
