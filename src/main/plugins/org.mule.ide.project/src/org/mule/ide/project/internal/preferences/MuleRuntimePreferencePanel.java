/*
 * $Id: MuleRuntimePreferencePanel.java 1 2007-08-16 16:30:25Z quoc $
 * --------------------------------------------------------------------------------------
...
public class MuleRuntimePreferencePanel extends Composite {
	...
	private void createDistroGroup() {
		...
		gridData = new GridData(GridData.FILL, SWT.CENTER, false, false);
		buttonAddJar.setLayoutData(gridData);
		buttonAddJar
				.addSelectionListener(new org.eclipse.swt.events.SelectionAdapter() {
					public void widgetSelected(org.eclipse.swt.events.SelectionEvent e) {
						 addJarDistribution();
	...
	private void setNewDefault(int idx) {
		// clear old checkmark
		if (idx != defaultIndex && defaultIndex > 0 && idx < tableRuntimes.getItemCount()) tableRuntimes.getItem(defaultIndex).setChecked(false);
	...
	private void doAddRuntimeDirectory(String filepath) {
		...
		if (pathList.contains(filepath)) {
			MessageDialog.openInformation(parentPage.getShell(), "Existing Mule Runtime", "Runtime directory has already been configured.");
			return;
		}

		IStatus status = MuleRuntime.validateRuntimeDirectory(path);
		if (! status.isOK()) {
			MessageDialog.openError(parentPage.getShell(), "Invalid Mule Runtime Directory", status.getMessage());
			return;			
		}
