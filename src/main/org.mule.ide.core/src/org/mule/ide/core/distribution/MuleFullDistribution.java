	...
	public void gatherSamples(File dir, Collection dest) {
		try {
			...
			if (nl.getLength() > 0) {
				...
				// Exclude the non-mule loanbroker projects
				if (name.startsWith("Loan Broker Example: ")) {
					return;
				}
				
				// Workaround for JIRA MULE-2612
				if (name.equals("Web App Example")) {
					return;
				}
				
				// This project is throwing an NPE on load, need to investigate why.
				if (name.startsWith("Loan Broker BPM Example")) {
					return;
				}
				...
				// Workaround for JIRA MULE-2612
				if (name.equals("Error Handler Example")) {
					dependencies.add(getMuleModule("mule-transport-file".substring(MULE_PREFIX.length())));
				} else if (name.equals("Hello Example")) {
					dependencies.add(getMuleModule("mule-transport-stream".substring(MULE_PREFIX.length())));
					dependencies.add(getMuleModule("mule-transport-vm".substring(MULE_PREFIX.length())));
				} else if (name.equals("Loan Broker ESN Example")) {
					dependencies.add(getMuleModule("mule-transport-vm".substring(MULE_PREFIX.length())));
				}
